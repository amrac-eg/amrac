...
const EditServices = ({ Services }: { Services: Services }) => {
  ...
  const [state, action, pending] = useActionState(
    UpdateServices.bind(null, Services.id , Services.publicId ?? ""),
    initialState
  );
  useEffect(() => {
          ...
          <form action={action} className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              {getFormFields().map((field) => {
                const fieldValue = Services[field.name as keyof Services];
